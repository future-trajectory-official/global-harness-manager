...
- スパイクWPは、技術的検証・調査・プロトタイピングに特化した一時的なWork Packageである。
- スパイクWPの成果物は「調査レポート」であり、本実装WPの計画策定に必要な情報（採用技術の決定、アーキテクチャの確定、リスクの特定等）を提供する。

#### 2. 運用ルール

- **発行タイミング**: スプリントプランニング時に、Work
  Packageに技術的不確実性が認められる場合、POの判断によりスパイクWPとして分離する。
- **スパイク対象の命名**: スパイク完了後に正式化された本実装WPは `WP_M'`（プライム）と表記する。
...
- スパイクWPの調査レポートは、後続の本実装WP（`WP_M'`）のバックログ内に
