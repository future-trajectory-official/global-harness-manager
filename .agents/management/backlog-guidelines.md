...
- **WPの命名規則**:
  ...
  - `WP_N` （Nは任意の数字）: 技術的不確実性が高いため事前調査を必要とするスパイクWP
  - `WP_M'` （プライム）:
    スパイクWP（WP_N）の調査結果を踏まえて正式化された本実装WP。スパイクにより後続WPの内容が変化した場合に用いる
...
技術的不確実性が高く、見積もりや実装計画の策定が困難なWork
Packageについては、本実装の前に「スパイクWP」として独立させた事前調査を実施する。

#### 1. スパイクWPの定義と目的

- **スパイクWP** (`WP_N` 表記) は、技術的検証・調査・プロトタイピングに特化した一時的なWork
  Packageである。
- スパイクWPの成果物は「調査レポート」であり、本実装WPの計画策定に必要な情報（採用技術の決定、アーキテクチャの確定、リスクの特定等）を提供する。
- スパイクWPのEffort見積は原則 **1回（1セッション）** とし、1セッションで完了可能な範囲に抑える。

#### 2. 運用ルール

- **発行タイミング**: `identify-work-package` または `session-planning` 時に、Work
  Packageに技術的不確実性が認められる場合、POの判断によりスパイクWPとして分離する。
- **スパイク対象の命名**: スパイク対象とする本実装WPを `WP_M` とした場合、そのスパイクWPを
  `WP_N`（Nは任意の番号）とし、スパイク後に正式化された本実装WPを `WP_M'`（プライム）と表記する。
- **調査範囲の明示**: スパイクWPのACには調査範囲・検証項目・合格基準を明記し、際限のない調査を防ぐ。
- **本実装WPへの移行条件**: スパイクWPの完了をもって、`WP_M'`（本実装WP）の `session-planning`
  を開始可能とする。スパイクWPの成果物（調査レポート）は `WP_M'`
  のバックログに参照リンクとして明記する。
...
- `session-planning` スキルは、`WP_M'`
  の計画策定時にこの参照リンクを確認し、調査結果を計画に反映することを必須とする。

#### 4. 乖離シグナル分析指標

`WP_M'`（本実装WP）の計画前見積と完了時実績の乖離が大きい（例:
見積もり介入回数の2倍以上）場合、その乖離はスパイクが不十分であった可能性を示すシグナルとして記録する。これは将来のスパイクWP発行判断の改善材料として活用する。

#### 5. 命名規則の具体例

| スパイク対象WP                | スパイクWP                        | スパイク後の本実装WP                              | 概要                                                                                                   |
| ----------------------------- | --------------------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `WP_1: 新APIクライアント実装` | `WP_2: 新APIクライアント技術検証` | `WP_1': 新APIクライアント実装（axios採用確定後）` | 使用するHTTPライブラリの選定が不確実なため、スパイクWPで技術検証を実施。結果を踏まえて本実装を正式化。 |
| `WP_3: 分散キャッシュ導入`    | `WP_4: 分散キャッシュ技術比較`    | `WP_3': 分散キャッシュ導入（Redis採用確定後）`    | Redis/Memcachedの選定が不確実なため、スパイクWPで比較検証。選定結果を踏まえて本実装を正式化。          |
