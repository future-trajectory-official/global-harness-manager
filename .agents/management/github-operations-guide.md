...
| --------------------------------------- | ---------------------------------------------------------------------------- | ---------------------------- |
| プロジェクトビジョン / プロダクトゴール | リポジトリ内 `VISION.md` / `product-backlog.md` で管理（GitHub Issue対象外） | `/kickoff` 実行時            |
...
3. PBI Issue の Status フィールドを WIP に更新する
...
1. **スプリント開始時**:
   「スプリントバックログ」ビューで今回のスコープを一覧し、POとコミット範囲を最終確認します。
2. **セッション開始時**: 該当PBIの Status を手動で WIP
   に変更し、着手を可視化します（将来のワークフロー自動化対象）。
3. **日次確認**: 「進行中PBI」ビューで長期間 WIP のままのPBIを特定し、POに状況を報告します。
4. **スプリント終了時**:
   「完了PBI」ビューで今スプリントの成果を一覧し、未完了PBIは次スプリントに繰り越し判断します。

**運用のポイント**:

- PBI Issue の Status フィールドは原則として手動操作不要（ワークフロー内で自動更新）。
