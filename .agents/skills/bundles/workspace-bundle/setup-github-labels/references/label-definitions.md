...
| ラベル        | 色        | 説明                   | 使用タイミング                        | 対象   |
| ------------- | --------- | ---------------------- | ------------------------------------- | ------ |
| `status:IDEA` | `#c5def5` | 未精査のバックログ候補 | 新規PBI Issue作成時にデフォルトで付与 | PBI    |
| `status:TODO` | `#f9c513` | 実装予定               | スプリント計画時にIDEAから昇格        | PBI/WP |
| `status:WIP`  | `#f66a0a` | 作業中                 | 実装着手時にTODOから変更              | PBI/WP |
| `status:DONE` | `#6f42c1` | 完了                   | 子WPが全てDONEになったらPBIをDONEに   | PBI/WP |

**Note**: `DONE` になったIssueはスプリント終了時に **Close（クローズ）** してアーカイブする。Close =
アーカイブ。
