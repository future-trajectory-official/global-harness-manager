...
2. **要件定義・壁打ち (Refine)**
   - **【PBI深掘りディスカッション】**
     ...
     詳細な問いかけ例と運用ガイドは [references/pbi-deep-dive.md](references/pbi-deep-dive.md)
     を参照。 深掘りの結果、技術的不確実性が高い場合は `WP_0`（スパイクWP）の分離を
     `session-planning`
     フェーズで提案します（スパイクWPは品質投資・不確実性の分離として位置づけます）。
