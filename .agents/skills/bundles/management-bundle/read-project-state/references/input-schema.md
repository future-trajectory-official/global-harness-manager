...
| Sprint             | 対象外 | あり | 単一インスタンス。search は不可。code 省略時は最新オープンを表示     |
| Epic               | あり   | あり |                                                                      |
| Feature            | あり   | あり |                                                                      |
...
- **search 対象外 Entity**（Vision / ProductGoal / Sprint）を search で呼んだ場合:
...
| state        | search   | string | Issue の状態（open / closed）                     |
