...
Deno.test("migrate-to-github --migrate --dry-run should show migration plan with WPs", async () => {
  const tmpFile = await Deno.makeTempFile({ suffix: ".md" });
  try {
    await Deno.writeTextFile(tmpFile, mockBacklog);
    ...
    assertStringIncludes(output, "Labels: type:PBI");
...
Deno.test("migrate-to-github --migrate --dry-run should show migration plan for DONE WP", async () => {
  const tmpFile = await Deno.makeTempFile({ suffix: ".md" });
  try {
    await Deno.writeTextFile(tmpFile, mockBacklog);
...
Deno.test("migrate-to-github --migrate --dry-run with --no-effort should suppress Effort Fields output", async () => {
  const tmpFile = await Deno.makeTempFile({ suffix: ".md" });
  try {
    await Deno.writeTextFile(tmpFile, mockBacklog);
