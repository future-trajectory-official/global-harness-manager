...
## 1. SSH エイリアス方式
複数の GitHub アカウントを同一マシンで使い分けるため、`~/.ssh/config` に「ホストエイリアス」を設定しています。
...
### なぜこの方式なのか？
- `git clone git@github.com:...` とすると、SSH クライアントはデフォルトの鍵（通常は最初に登録されたもの）を送信してしまい、意図しないアカウントで認証されることがあります。
...
この設定がないと、SSH クライアントはエージェントが持っている全ての鍵を順番に試そうとします。GitHub のログイン試行回数制限に引っかかるのを防ぐため、このオプションは必須です。
