---
description: スプリントの開始プロセス（プロダクトゴール確認、プロダクトバックログリファインメント、分類階層改善、スプリント開始宣言、スプリントプランニング、作業分解、レビュー計画）を安全に1ステップずつ実行するワークフロー。
---
...
- **期待される結果（終了条件）**:
  ...
  3. `/sprint-start` ワークフローの全工程完了が宣言されること。
