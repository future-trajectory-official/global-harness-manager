---
description: スプリントの終了プロセス（レビュー、メトリクス・予実評価、KPT、自己スキル最適化、ステートレスリセット）を安全に1ステップずつ実行するワークフロー。
---
...
## Phase 2: スプリントメトリクス定量評価・予実分析 (Metrics & Estimation Variance)
...
> [!IMPORTANT]
> 上記の「期待される結果」を満たすエビデンスを提示し、**「Phase
> 2が完了しました。定量的な評価および予実差の確定を行いました。よろしければ『次のフェーズ（Phase
> 3）へ進む』とご指示ください」** と明確にプロンプトして停止してください。

<!-- STOP -->

---

## Phase 3: スプリントレトロスペクティブ (Retrospective KPT)
...
- **入力（前提条件）**: Phase 2 で確定した定量評価スコアおよび予実差分析レポート。
- **手続き**:
...
## Phase 4: 自己スキルオプティマイザー (Skill Optimization)
...
> [!IMPORTANT]
> 上記の「期待される結果」を満たすエビデンスを提示し、**「Phase
> 4が完了しました。スキルのクリーンアップ・最適化案が提示されました。よろしければ『次のフェーズ（Phase
...
## Phase 5: ステートレスリセットの検討 (Stateless Reset)
...
> [!IMPORTANT]
> 完了後、**「/sprint-end
> ワークフローがすべて完了しました！スプリント全体の儀式、メトリクス評価、スキル最適化、および環境のリセットが完了しました。次回スプリントはクリーンな状態で、新たに
