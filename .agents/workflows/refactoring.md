...
- **セルフチェック**:
  - [ ] **[儀式]** ワークフローの各ステップにおける役割と、その「制約」をすべて復唱し、遵守を誓約したか。
...
- **セルフチェック**:
  ...
  - [ ] **[中止判断]** テストがない、または既に失敗している場合は、**リファクタリングを中止し、オーケストレーター（ユーザー）に「テストの作成または修復」を優先するよう報告したか。**
...
- **ロール**: `@[tester]` (すべての制約を遵守)
- **セルフチェック**:
  - [ ] **[原則: Verification Integrity]** プロジェクト全体の全テストがグリーンであることを確認したか。
