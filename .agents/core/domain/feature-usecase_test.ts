...
Deno.test("featureUseCase - define should return Plan with create operation", () => {
  ...
  assertEquals(plan.steps[1].params.title, "Login");
});
...
Deno.test("featureUseCase - revise should succeed with code even if id is undefined", () => {
  const plan = featureUseCase.revise(
    makeId({ id: undefined, code: "99" }),
    makeStatement(),
    makeReason(),
  );
  assertEquals(plan.steps.length, 3);
...
Deno.test("featureUseCase - unassignFromEpic should return Plan with update", () => {
  ...
  assertEquals(plan.steps[0].operation, "resolve");
  assertEquals(plan.steps[1].operation, "update");
  assertEquals(plan.steps[1].params.parentEpic, undefined);
});
 ...
 * @verify code を保持したまま find が Plan を返し、view 操作を含むこと
 */
Deno.test("featureUseCase - find should succeed with code even if id is undefined", () => {
  const plan = featureUseCase.find(makeId({ id: undefined, code: "42" }));
  assertEquals(plan.summary, "Find feature: Login");
  assertEquals(plan.steps[0].entity, "Scope");
  assertEquals(plan.steps[0].operation, "resolve");
  assertEquals(plan.steps[1].operation, "view");
 ...
 */
Deno.test("featureUseCase - find should throw when both id and code are undefined", () => {
  assertThrows(
