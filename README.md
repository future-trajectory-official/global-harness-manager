...
- **タスク開発**: `/session-start` で承認された `task.md` のチェックリストを進行 (または
  `/refactoring`)
  - 承認済みの
...
|                    | `> 「task.md に沿って実装を進めて」` | ACベースの実装ループ（task.md駆動）                        | タスク着手時           |
...
- `.opencode/`: ハーネスの中核となるエージェント資源（単一ルート）。
  - `skills/bundles/`: 機能ごとにカプセル化されたスキル群。
    ...
    - `git-bundle`: ブランチ・コミット・PR等のGit運用に関わるスキル。
    - `meta-bundle`: エージェント自身の最適化や文脈同期に関わるスキル。
  - `agents/`: 各ロール（専門家）の定義。
  - `commands/`: タスク完遂までの標準化された手順書（ワークフロー入口）。
  - `context/`: 管理概念・プロジェクト固有の用語集。
